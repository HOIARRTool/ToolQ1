...
def display_admin_page():
    ...
    # (เราจะนิยามฟังก์ชันนี้ไว้ข้างใน เพื่อให้แน่ใจว่ามันพร้อมใช้งาน)
    def convert_be_str_to_ad_str(be_date_str):
        ...
        """
        # ใช้ re.search เพื่อค้นหา pattern วันที่
        match = re.search(r'(\d{1,2})/(\d{1,2})/(\d{4})', str(be_date_str))
        
        if match:
            # ดึงส่วน วัน, เดือน, ปี(พ.ศ.) ออกมา
            day_str, month_str, be_year_str = match.group(1), match.group(2), match.group(3)
            
            try:
                be_year = int(be_year_str)
                
                # ตรวจสอบว่าเป็นปี พ.ศ. (เช่น มากกว่า 2500)
                if be_year > 2500:
                    ad_year = be_year - 543
                    # แทนที่ *เฉพาะ* ปี พ.ศ. ที่เจอในสตริง
                    ad_date_str = be_date_str.replace(be_year_str, str(ad_year), 1)
                    return ad_date_str
            except ValueError:
                # หากแปลงตัวเลขไม่สำเร็จ
                return be_date_str
        
        # หากไม่ตรง pattern หรือไม่ใช่ปี พ.ศ. ให้คืนค่าเดิม
        return be_date_str
    # --- จบฟังก์ชันช่วย ---
    ...
    with st.spinner("กำลังประมวลผลไฟล์ กรุณารอสักครู่..."):
        ...
        # ---------------- แปลงวันที่ ----------------
        # 9. ✅ *** ส่วนที่แก้ไขตรรกะการแปลงวันที่ (รองรับ B.E.) ***
        
        try:
            # 1. Clean สตริง: ลบ ' น.' (และ space ที่อาจอยู่หน้า) และ .strip()
            cleaned_date_str_be = df['Occurrence Date'].astype(str).str.replace(r'\s*น\.', '', regex=True).str.strip()
            
            # 2. แปลงสตริงปี พ.ศ. เป็น ค.ศ. โดยใช้ฟังก์ชันช่วย
        ...
        # 3. ลอง parse format "dd/mm/yyyy HH:MM" (ไม่มีวินาที)
        ...
        # 4. ลอง parse format "dd/mm/yyyy HH:MM:SS" (มีวินาที)
        ...
        # 5. รวมผลลัพธ์: เอาค่าจาก 1, ถ้าล้มเหลว (NaT) ให้ลองเอาค่าจาก 2
        ...
        if invalid_dates > 0:
            # แสดงสตริง *ก่อน* การแปลง (ฉบับ พ.ศ.) ที่ทำให้เกิดปัญหา
