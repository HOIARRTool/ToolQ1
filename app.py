...
def display_user_guide():
    ...
    **ขั้นตอน:**
    ...
    | **Incidents Analysis** | **ติดตามความคืบหน้าในการแก้ไขปัญหา**: การวิเคราะห์อุบัติการณ์: จากรายงานอุบัติการณ์ในแต่ละหมวดหมู่ (โดยเฉพาะ PSG9) เกิดขึ้นในระดับความรุนแรงใดบ้าง ถูกแก้ไขไปแล้วกี่เปอร์เซ็นต์ และมีเคสใดค้างอยู่บ้างเพื่อทำการติดตามได้อย่างครอบคลุม  |
...
def display_admin_page():
    ...
    # บรรทัดนี้จะสร้างตัวแปร uploaded_file ขึ้นมาก่อน
    uploaded_file = st.file_uploader("เลือกไฟล์ของคุณที่นี่:", type=[".xlsx"])
    
    # จากนั้นจึงนำตัวแปรมาตรวจสอบในบรรทัดนี้
    ...
    with st.spinner("กำลังประมวลผลไฟล์ กรุณารอสักครู่..."):
        ...
        # ---------------- ตรวจคอลัมน์จำเป็น ----------------
        required_source_cols = ["Incident", "วดป.ที่เกิด", "ความรุนแรง", "สถานะ"]
        missing_source_cols = [key for key in required_source_cols if key not in df.columns]
        if missing_source_cols:
            st.error(f"ไม่พบคอลัมน์ที่จำเป็นในไฟล์ Excel: {', '.join(missing_source_cols)}")
            st.stop()
        ...
        df.rename(columns={
            ...
            "ความรุนแรง": "Impact",
            "Incident": "รหัส: เรื่องอุบัติการณ์"
        }, inplace=True)
