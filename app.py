...
# === Injected: assistants & anonymizer imports ===
try:
    from anonymizer import load_ner_model, anonymize_text
except Exception as _e:
    load_ner_model = None
    def anonymize_text(text, ner_model=None):
        return text

try:
    from risk_register_assistant import get_risk_register_consultation
except Exception as _e:
    def get_risk_register_consultation(*args, **kwargs):
        return {"error": "risk_register_assistant ไม่พร้อมใช้งาน"}

# ai_assistant is optional; we will probe its functions at runtime
try:
...
# === Injected: NER model (cached) ===
try:
    if 'NER_PIPELINE' not in st.session_state:
        NER_PIPELINE = load_ner_model() if load_ner_model else None
        st.session_state['NER_PIPELINE'] = NER_PIPELINE
    else:
        NER_PIPELINE = st.session_state.get('NER_PIPELINE', None)
except Exception as _e:
    NER_PIPELINE = None
st.markdown("""
<style>
...
def display_admin_page():
    ...
    def convert_be_str_to_ad_str(be_date_str):
        ...
        """
        match = re.search(r'(\d{1,2})/(\d{1,2})/(\d{4})', str(be_date_str))
        
        if match:
            day_str, month_str, be_year_str = match.group(1), match.group(2), match.group(3)
            
            try:
                be_year = int(be_year_str)
                if be_year > 2500:
                    ad_year = be_year - 543
                    ad_date_str = be_date_str.replace(be_year_str, str(ad_year), 1)
                    return ad_date_str
            except ValueError:
                return be_date_str
    ...
    with st.spinner("กำลังประมวลผลไฟล์ กรุณารอสักครู่..."):
        ...
        df.rename(columns={
            ...
            "หมวด": "หมวด",
            "สถานที่เกิดเหตุ": "ชนิดสถานที่",
            "การดำเนินการ/การแก้ไขที่ได้ดำเนินการไปแล้ว": "Raw Actions",
            "สรุปปัญหา/เหตุการณ์โดยย่อ": "รายละเอียดการเกิด"
            # (คอลัมน์อื่นๆ เช่น self_report จะถูกเก็บชื่อเดิมไว้ก่อน)
        }, inplace=True)
        ...
        # (ส่วนนี้คือส่วนที่เติม 'None' (str) เข้าไปในคอลัมน์ที่เป็นตัวเลขด้วย)
        ...
        # ---------------- แปลงวันที่ ----------------
        try:
            cleaned_date_str_be = df['Occurrence Date'].astype(str).str.replace(r'\s*น\.', '', regex=True).str.strip()
            cleaned_date_str_ad = cleaned_date_str_be.apply(convert_be_str_to_ad_str)
        except Exception as e:
            st.error(f"เกิดข้อผิดพลาดขณะเตรียมข้อมูลวันที่: {e}")
            st.stop()
        ...
        if invalid_dates > 0:
            example_failed_date = cleaned_date_str_be[invalid_mask].iloc[0] 
        ...
        if df.empty:
            st.error("ไม่พบข้อมูลที่มี 'วันที่' ที่ถูกต้องเลยหลังการกรอง (อาจเกิดจากทุกแถวมีวันที่ผิดรูปแบบ)")
            st.stop()
        ...
        # ---------------- (เพิ่ม) จัดการ Type ก่อนบันทึก ----------------
        # ✅ *** นี่คือส่วนที่เพิ่มเข้ามาเพื่อแก้ปัญหา ***
        # แก้ไขปัญหา "Could not convert 'None' with type str"
        # คอลัมน์เหล่านี้ควรเป็นตัวเลข แต่มี 'None' (str) ปน
        
        cols_to_convert = ['self_report', 'potential_harm']
        for col in cols_to_convert:
            if col in df.columns:
                # errors='coerce' จะเปลี่ยน 'None' (str) และค่าที่แปลงไม่ได้ ให้เป็น NaT/NaN
                # ซึ่ง Parquet สามารถจัดเก็บได้ (มักจะเก็บเป็น float ที่มี NaN)
...
if __name__ == "__main__":
    ...
    elif selected_analysis == "NER / Anonymizer":
        st.markdown("<h4 style='color:#001f3f;'>NER / Anonymizer</h4>", unsafe_allow_html=True)
        st.info("ปกปิดข้อมูลอ่อนไหว (เช่น HN, PERSON, LOCATION, ORGANIZATION) จากข้อความของคุณโดยอัตโนมัติ")

        text_in = st.text_area("วางข้อความที่ต้องการปกปิดข้อมูล (Thai/English) :", height=180)
        col_ner1, col_ner2 = st.columns([1,1])
        with col_ner1:
            run_btn = st.button("🔐 ทำการปกปิดข้อมูล (Anonymize)", type="primary", use_container_width=True)
        with col_ner2:
            show_model = st.checkbox("แสดงสถานะโมเดล NER", value=False)

        if show_model:
            st.caption(f"NER pipeline loaded: {'✅ Ready' if NER_PIPELINE else '⚠️ Not available'}")

        if run_btn and text_in.strip():
            try:
                out = anonymize_text(text_in, NER_PIPELINE)
                st.success("ดำเนินการสำเร็จ")
                st.text_area("ผลลัพธ์ (anonymized)", value=out, height=180)
            except Exception as e:
                st.error(f"เกิดข้อผิดพลาดระหว่างทำ anonymize: {e}")

    elif selected_analysis == "RCA Assistant":
        st.markdown("<h4 style='color:#001f3f;'>RCA Assistant</h4>", unsafe_allow_html=True)
        st.info("ผู้ช่วยวิเคราะห์สาเหตุราก (Root Cause Analysis). จะพยายามเรียกใช้ ai_assistant ถ้ามี")

        prompt_rca = st.text_area("คำถาม/ปัญหาที่ต้องการให้ช่วยวิเคราะห์ (บริบท: ใช้ข้อมูลช่วงที่คุณเลือกทางซ้าย):", height=160)
        run_rca = st.button("🤖 วิเคราะห์ RCA", type="primary")

        if run_rca:
            try:
                response_text = None
                # ส่งบริบทขั้นต่ำไปให้โมดูล AI (ถ้ามี)
                context = {
                    "n_records": int(df_filtered.shape[0]),
                    "period": (min_date_str, max_date_str),
                    "top_codes": df_filtered['รหัส'].value_counts().head(10).to_dict()
                }
                if _ai_mod is not None:
                    # พยายามเรียกตามชื่อฟังก์ชันที่พบบ่อย
                    if hasattr(_ai_mod, "get_rca_response"):
                        response_text = _ai_mod.get_rca_response(prompt_rca, df_filtered, context)
                    elif hasattr(_ai_mod, "chat"):
                        response_text = _ai_mod.chat(prompt_rca, context=context)
                    elif hasattr(_ai_mod, "assist"):
                        response_text = _ai_mod.assist(prompt_rca, context=context)
                if not response_text:
                    response_text = "⚠️ ไม่พบโมดูลหรือฟังก์ชัน RCA ที่พร้อมใช้งานใน ai_assistant.py กรุณาตรวจสอบชื่อฟังก์ชัน เช่น get_rca_response()/chat()/assist"
                st.markdown(response_text)
            except Exception as e:
                st.error(f"เกิดข้อผิดพลาดระหว่างเรียก RCA assistant: {e}")

    elif selected_analysis == "Risk Register Assistant":
        st.markdown("<h4 style='color:#001f3f;'>Risk Register Assistant</h4>", unsafe_allow_html=True)
        st.info("ค้นหารหัส/ชื่ออุบัติการณ์และดึงข้อมูลประกอบเพื่อช่วยบันทึก Risk Register")

        # โหลด/อัปโหลดไฟล์มาตรการความเสี่ยง (mitigation)
        st.markdown("**ไฟล์มาตรการป้องกันและการติดตาม (risk_mitigation.xlsx)**")
        up_mit = st.file_uploader("เลือกไฟล์ (คอลัมน์ที่สำคัญ: 'รหัส','มาตรการป้องกันและถ่ายโอนความเสี่ยง','การติดตาม')", type=["xlsx"], key="mit_upl")
        mitigation_df = None
        try:
            import pandas as _pd
            default_mit = Path("risk_mitigation.xlsx")
            if up_mit is not None:
                mitigation_df = _pd.read_excel(up_mit)
            elif default_mit.exists():
                mitigation_df = _pd.read_excel(default_mit)
        except Exception as e:
            st.warning(f"อ่านไฟล์ risk_mitigation.xlsx ไม่สำเร็จ: {e}")

        query = st.text_input("พิมพ์รหัสหรือชื่ออุบัติการณ์ที่ต้องการค้นหา (เช่น CPP101 หรือ 'medication')")
        run_rr = st.button("📋 สร้างสรุปสำหรับ Risk Register", type="primary")

        if run_rr and query.strip():
            try:
                res = get_risk_register_consultation(query, df_filtered, mitigation_df if mitigation_df is not None else _pd.DataFrame())
                if 'error' in res:
                    st.error(res['error'])
                else:
                    c1, c2, c3 = st.columns(3)
                    c1.metric("รหัส", res.get("incident_code","-"))
                    c2.metric("ชื่อ", res.get("incident_name","-"))
                    c3.metric("จำนวนครั้ง", f"{res.get('total_occurrences',0):,}")

                    c4, c5, c6 = st.columns(3)
                    c4.metric("Impact สูงสุด (1-5)", str(res.get("max_impact_level","-")))
                    c5.metric("Frequency Level (1-5)", str(res.get("frequency_level","-")))
                    c6.metric("Risk Level", res.get("risk_level_code","-"))
                    st.info(f"Risk Category: **{res.get('risk_category','-')}**")

                    st.markdown("**มาตรการป้องกันที่มีอยู่ (Existing Prevention):**")
                    st.write(res.get("existing_prevention","-"))

                    st.markdown("**การติดตาม/ดัชนีเฝ้าระวัง (Monitoring):**")
                    st.write(res.get("existing_monitor","-"))

                    inc_df = res.get("incident_df")
                    if inc_df is not None and len(inc_df) > 0:
                        st.markdown("---")
                        st.markdown("**รายการเหตุการณ์ที่เกี่ยวข้อง (ในช่วงที่คุณเลือก):**")
                        st.dataframe(inc_df[['Occurrence Date','รหัส','ชื่ออุบัติการณ์ความเสี่ยง','Impact','Impact Level']],
                                     use_container_width=True, hide_index=True)
            except Exception as e:
                st.error(f"เกิดข้อผิดพลาดใน Risk Register Assistant: {e}")
